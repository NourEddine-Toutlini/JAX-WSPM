...
def convert_to_csr(bcoo_matrix):
    """Convert BCOO matrix to CSR format."""
    indices = bcoo_matrix.indices
    data = bcoo_matrix.data
    shape = bcoo_matrix.shape
    return sparse.BCSR((data, indices), shape=shape)

def optimized_matrix_formation(Global_mass, Capacity_m, Global_stiff, dt):
    """Optimized version of system matrix formation using CSR format."""
    # Convert matrices to CSR format
    mass_csr = convert_to_csr(Global_mass)
    capacity_csr = convert_to_csr(Capacity_m)
    stiff_csr = convert_to_csr(Global_stiff)
    
    # Perform operations in CSR format
    temp = mass_csr @ capacity_csr
    temp = sparse.scale_csr(temp, 1/dt)
    result = sparse.add_csr(temp, stiff_csr)
    
    # Convert back to BCOO if needed
    return result.to_bcoo()
...
#                 # track_memory("Before boundary conditions")
...
#                     self.w_1d
#                 )
#                 # track_memory("After boundary conditions")
...
#         return pressure_head_new, error, iter_count
    ...
    def solve_richards_step(self, pressure_head_n: jnp.ndarray, thetan_0: jnp.ndarray, dt: float) -> Tuple[jnp.ndarray, float, int]:
        ...
        def body_fun(carry):
            pressure_head_prev, pressure_head_m, err, iter_count = carry
            ...
            return pressure_head_m, pressure_head_new, error, iter_count + 1
        ...
        _, pressure_head_new, err, iter_count = final_carry
    ...
    def solve(self) -> Dict[str, Any]:
        ...
        while current_time < self.config.time.Tmax:
            ...
            # Ensure computation is finished before stopping the timer
            pressure_head.block_until_ready()

            # simulation_time = time.perf_counter() - simulation_start
            # print(f"Simulation time for the Richards solver: {simulation_time:.6f} seconds")
            
            _, _, theta = vmap(van_genuchten_model, in_axes=(0, None))(
