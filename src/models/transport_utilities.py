...
#Here we calculate the flux without Automatic Df:
@jit
def get_elements_with_node(triangles, node_index):
    return jnp.any(triangles == node_index, axis=1)

@jit
def count_elements_with_node(triangles, node_index):
    return jnp.sum(jnp.any(triangles == node_index, axis=1))
@jit
def calculate_fluxes_and_dispersion_FE(points: jnp.ndarray,
                                  triangles: jnp.ndarray,
                                  pressure_head: jnp.ndarray,
                                  phi: jnp.ndarray,
                                  DL: float,
    ...
    Uses an element-based gradient calculation instead of interpolation.
    ...
    theta_s = phi[1]  # Saturated water content
    
    def process_node(node):
        elements_mask = get_elements_with_node(triangles, node)
        count = jnp.sum(elements_mask)  # Number of connected elements

        def process_element(element, mask):
            nodes = triangles[element]
            coords = points[nodes]
            local_pressure_head = pressure_head[nodes]

            x0, y0 = coords[0]
            x1, y1 = coords[1]
            x2, y2 = coords[2]

            b1, b2, b3 = y1 - y2, y2 - y0, y0 - y1
            c1, c2, c3 = x2 - x1, x0 - x2, x1 - x0

            Aire = jnp.abs(c3 * b2 - c2 * b3) / 2

            # Compute local gradients ?h = [dh/dx, dh/dz]
            dh_dx = (b1 * local_pressure_head[0] + b2 * local_pressure_head[1] + b3 * local_pressure_head[2]) / (2 * Aire)
            dh_dz = (c1 * local_pressure_head[0] + c2 * local_pressure_head[1] + c3 * local_pressure_head[2]) / (2 * Aire)

            return jnp.where(mask, dh_dx, 0.), jnp.where(mask, dh_dz, 0.)

        # Compute element-wise sums
        sums = vmap(process_element)(jnp.arange(triangles.shape[0]), elements_mask)
        local_dh_dx = jnp.sum(sums[0]) / count
        local_dh_dz = jnp.sum(sums[1]) / count

        # Get conductivity and water content using van Genuchten model
        _, K, theta = van_genuchten_model(pressure_head[node], phi)

        # Compute water fluxes
        water_flux_x = -K * local_dh_dx
        water_flux_z = -K * (local_dh_dz + 1)  # +1 for gravity

        abs_q = jnp.sqrt(water_flux_x**2 + water_flux_z**2)

        # Calculate dispersion tensor components
        tau = (theta**(7/3)) / theta_s**2
        Dispersion_xx = ((DL * water_flux_x**2 + DT * water_flux_z**2) / abs_q + theta * Dm * tau)
        Dispersion_zz = ((DL * water_flux_z**2 + DT * water_flux_x**2) / abs_q + theta * Dm * tau)
        Dispersion_xz = (DL - DT) * water_flux_x * water_flux_z / abs_q

        return water_flux_x, water_flux_z, abs_q, Dispersion_xx, Dispersion_xz, Dispersion_zz, theta
